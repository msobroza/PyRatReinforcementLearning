...
def save_checkpoint(state, filename='checkpoint2.pth.tar'):
...
class Environment:
    ...
    def record_step(self):
        ...
        grid = self.grid.grid
        #visible_player = np.array(self.grid.visible(self.agent_player.pos))
        #visible_opponent = np.array(self.grid.visible(self.agent_opponent.pos))
    ...
    def visible_state(self):
        ...
        #extras = np.array([self.agent_player.health, self.agent_opponent.health, self.agent_player.score, self.agent_opponent.score , yp, xp, yo, xo])
        #extras = np.array([yp, xp])
        #visible_opponent = np.array(self.grid.visible(self.agent_opponent.pos))
        #return np.concatenate((visible_player.flatten(), extras), 0)
